...
/*   Updated: 2024/11/06 10:21:04 by eandre           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// First, login into the server. This part is pretty easy, because you just need to send the
// informations given in the irc norm.
...
// And second is trying to find EVERY errors possible, but with added security, with the last if
// being a full stop to everything outside of the greating of the server
...
{
    return (msg == ((":42chan 464 ") + bot_name +
                    (" :Password is incorrect or was not provided\r\n")) ||
            msg == ":42chan 461 guest PASS :Not enough parameters\r\n");
}

bool Ai::is_name_incorrect()
{
    return (msg == ":42chan 432 guest " + bot_name +
                       " :Invalid nickname: Nickname contains invalid characters: #%:,*?!@. "
                       "\\t\\r\\n\r\n" ||
            msg == ":42chan 432 guest " + bot_name +
                       " :Invalid nickname: First character of nickname must be a-z or A-Z\r\n" ||
