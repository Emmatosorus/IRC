...
With the help of the popen function, which sends the curl into the terminal and do a redirection into a file,
we can call the ai and then access her answer.
Of course this is easier said than done, there is small things to understand to make this process smooth.
Each functions here is a step.
*/
...
//Second, put the message in the correct section of the curl command (the second content is to fill up with the actual message)
...
//Third, we need to execute the command, find any potential errors and get the correct line with the ai's message
...
//Finaly, we just need to get the message from the line we just got, and put it in the norm for an irc server (add PRIVMSG and the name of the sender to the ai message)
