...
The pm commands are only available in pm, which means the bot won't answer in channel message to any
of theses : botjoin, botjoink.

*/

// This function is the pm command hub, where every pm command is executed and also where every
// error message are sent.
...
int Guardian::manage_pm_request()
{
    ...
    {
        msg = "PRIVMSG " + sender_name +
              ": Error! This bot can't join multiple channels at once!\r\n";
        return (0);
    }
    else if (status == 0)
        return (0);
...
This bot doesnt allow for multiple join at a time so if you put a comma somewhere, an error will be
sent to you
*/
...
This function also allows you to ask (preferably gently) the bot to join a channel locked with a
key. To do so, you just need to put in the first argument the channel name, and the second the key
to this channel. Don't forget to put the # before the name ! This bot doesnt allow for multiple join
at a time so if you put a comma somewhere, an error will be sent to you
*/
