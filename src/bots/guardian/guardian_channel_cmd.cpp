...
The channel commands are only available in channels, which means the bot won't answer in private
message to any of theses : addword, rmword, cleanword, botleave.
...
// This function is the channel command hub, where every channel command is executed and also where
// every error message are sent.
...
If somebody tries to say a banned word, the bot will send a warning to operators of this channel
with the name of the sender. Operators will kill you if they see it, so please don't do it.
...
/*
And finaly, botleave.
If you want the bot to part a channel and leave nothing behind, use this command, if not, simply
kick him ! (ouch) This function takes no arguments.
*/
...
This isnt a function, its utility is simply to send the message to operators in case of someone
sending a bad word.
*/
...
int Guardian::does_msg_contain_badword()
{
    ...
    for (std::vector<std::string>::iterator it_msg = msg_split.begin(); it_msg != msg_split.end();
         it_msg++)
        for (std::vector<std::string>::iterator it_word = (*it_bw).words.begin();
             it_word != (*it_bw).words.end(); it_word++)
            if ((*it_msg).find(*it_word, 0) != std::string::npos)
                if ((*it_msg).length() == (*it_word).length())
                    return (msg =
                                "PRIVMSG @" + channel + ": BADWORD SAID BY " + sender_name + "\r\n",
                            TRUE);
