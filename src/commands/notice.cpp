...
void Server::_notice(PollfdIterator* it, const std::vector<std::string>& args)
{
	...
	for (size_t i = 0; i < targets.size(); i++)
	{
		...
		if (target_channel_it != m_channels.end())
		{
			...
			target_channel.send_msg_except(client.fd, ":" + client.nickname + " NOTICE " + target_channel.name + " :" + args[2]);
			continue;
		}
		...
		if (target_user_it != m_clients.end())
		{
			...
			target_user.send_msg(":" + client.nickname + " NOTICE " + target_user.nickname + " :" + args[2]);
			continue;
		}
