...
void Server::_privmsg(PollfdIterator* it, const std::vector<std::string>& args)
{
    ...
    for (size_t i = 0; i < targets.size(); i++)
    {
        ...
        if (target_channel_it != m_channels.end())
        {
			...
			if (is_to_operators)
			{
				target_channel.send_msg_to_operators(client.fd, ":" + client.nickname + " PRIVMSG " + target_channel.name + " :" + msg);
				continue;
			}
            target_channel.send_msg_except(client.fd, ":" + client.nickname + " PRIVMSG " + target_channel.name + " :" + msg);
            continue;
        }
